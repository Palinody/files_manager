...
def generate_file_info(file_paths:List[str], dirpath:str="") -> Tuple[int, float, str]:
    """
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if filter:
            ...
            if is_size_filer(filter):
                tree[dirpath] = [(size, timestamp, fn) for size, timestamp, fn in generate_file_info(filenames, dirpath=dirpath) if filter_size_to_logic(size, filter)]
            # or a timestamp comparator filter
            elif is_timestamp_filter(filter):
                tree[dirpath] = [(size, timestamp, fn) for size, timestamp, fn in generate_file_info(filenames, dirpath=dirpath) if filter_timestamp_to_logic(timestamp, filter)]
        else:
            tree[dirpath] = [*generate_file_info(filenames, dirpath=dirpath)]
        if not recursion:
            return tree
    return tree
...
def make_list_from_dict_tree(tree:Dict[str, List[Tuple[Any, Any, str]]]) -> List[Tuple[Any, Any, str]]:
    """
    ...
    for keys, values in tree.items():
        # skip empty folders
        if values:
            files_list += [(size, date, os.path.join(keys, fn)) for size, date, fn in values]
    return files_list
...
def format_file_info(file_info:Tuple[int, float, str]) -> Tuple[str, str, str]:
    ...
    return ("{:,d}".format(file_info[0]), 
            datetime.datetime.utcfromtimestamp(file_info[1]).strftime('%d-%m-%Y %H:%M:%S'),
            file_info[2])
...
def print_tree_with_sorting_args(tree:Dict[str, List[Tuple[int, float, str]]], sorting_rule:str=None) -> None:
    ...
    if not sorting_rule:
        ...
        if sorting_rule == "size":
            for file_info in sorted(make_list_from_dict_tree(tree), key=lambda x: x[0]):
                # print("{:15,d}".format(format_file_info(file_info)[0]) + " %s %s"%format_file_info(file_info)[1:])
                print(pretty_formatter%(format_file_info(file_info)))
        elif sorting_rule == "rSize":
            for file_info in sorted(make_list_from_dict_tree(tree), key=lambda x: x[0], reverse=True):
                # print("{:15,d}".format(format_file_info(file_info)[0]) + " %s %s"%format_file_info(file_info)[1:])
                print(pretty_formatter%(format_file_info(file_info)))
        elif sorting_rule == "date":
            for file_info in sorted(make_list_from_dict_tree(tree), key=lambda x: x[1]):
                print(pretty_formatter%(format_file_info(file_info)))
        elif sorting_rule == "rDate":
            for file_info in sorted(make_list_from_dict_tree(tree), key=lambda x: x[1], reverse=True):
                print(pretty_formatter%(format_file_info(file_info)))
...
def main():
    ...
    parser.add_argument("--root", nargs="?", help="Root directory from which we enable purge. This option is required.", required=True)
    parser.add_argument("--recursion", "-r", action="store_true", help="Recursion option.")
    ...
    parser.add_argument("--remove", action="store_true", help="Will remove all the files based on the tree that has been constructed.")
    
    parser.add_argument("--verbose", "-V", action="store_true", help="Shows which files have been selected from the chosen root dir.")
